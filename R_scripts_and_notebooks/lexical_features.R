...
for (i in 1:length(data_n$STATUS)) {
  ...
  data_n$Lexical_Diversity[[i]] <- round(quanteda::lexdiv(quanteda::dfm(data_n$STATUS[i], verbose = F, removeNumbers = F, removePunct = F, removeSeparators = F), measure = "TTR"), 3)
}
